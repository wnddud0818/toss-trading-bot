...
6. 장중 1분봉(현재 세션 분봉만 사용)에서 전일 고가 또는 장중 박스권을 거래량과 함께 돌파할 때만 진입합니다. 돌파·거래량 확인은 직전 완성 분봉으로 하고, 미완성 분봉은 돌파 유지 확인에만 씁니다.
7. VWAP 아래 돌파, 장중 과열, 과도한 갭상승, 상한가 근접 추격은 막습니다.
8. 손절, 트레일링 스탑, 이익 보호 스탑, 장중 급락, 시장 위험 회피, 최대 보유일로 청산합니다.
   - 이익 보호 구간(profit lock)에 도달한 승자는 최대 보유일에 강제 청산하지 않고 조여진 트레일링이 관리합니다.
   - 시장 위험 회피(risk-off) 시 미수익 포지션만 즉시 정리하고, 수익 포지션은 트레일링만 조입니다.
   - 청산된 종목은 재진입 쿨다운(KR 5일, US 10일) 동안 다시 사지 않습니다.
...
- `max_5d_return_pct`: 단기 급등주 배제 기준 (최근 1개월 내 5일 변화율 기준)
